...
读取时 Popup 或 Chrome Side Panel 会按需注入 parser 与 reader，因此扩展重新加载后不需要刷新已经打开的携程页面；城市、日期和位置/品牌/酒店关键词控制仅在用户点击对应按钮后操作携程真实候选；执行搜索时才点击携程真实搜索按钮；开始采集后才以有限步长自动滚动，并可随时停止。reported_total 仅作页面信息展示，不限制目标数量或完成条件。
